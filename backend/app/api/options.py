...
async def chain(
    ...
    return await _get_chain_live(symbol, expiry)
...
async def tshape(
    ...
    data = await _get_chain_live(symbol, expiry)
    chain = data["chain"]
    atm = data["atmStrike"]
...
async def term_structure(symbol: str = Query("NIFTY"), max_expiries: int = Query(6, ge=1, le=10)):
    ...
    for exp in data["expiries"][:max_expiries]:
        try:
            exp_data = data if exp == data["expiry"] else await _get_chain_live(symbol, exp)
...
async def strategies(
    ...
    if far:
        try:
            far_data = await _get_chain_live(symbol, far)
