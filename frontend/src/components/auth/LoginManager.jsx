    ...
    return (
      <div style={{display:'flex',gap:8,alignItems:'center'}}>
        <div aria-label={`Logged in as ${user.username}`} title={user.username} style={{display:'flex',gap:8,alignItems:'center', background:'rgba(255,255,255,0.06)', padding:'4px 10px 4px 4px', borderRadius:999, border:'1px solid rgba(255,255,255,0.08)'}}>
          <span style={{width:28,height:28,borderRadius:999, background:'linear-gradient(135deg,#2f8bff,#00e6a0)', display:'grid', placeItems:'center', fontWeight:800, fontSize:11, color:'#001a0c'}}>{user.avatar}</span>
          <span style={{fontSize:12, fontWeight:700, color:'#eef4ff'}}>{user.username}</span>
        </div>
          ...
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=> setOpen(false)} style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', backdropFilter:'blur(8px)', zIndex:60, display:'grid', placeItems:'center', padding:16}} aria-modal="true" role="dialog">
            <motion.div initial={{scale:0.96,y:8,opacity:0}} animate={{scale:1,y:0,opacity:1}} exit={{scale:0.96,opacity:0}} onClick={e=>e.stopPropagation()} style={{width:'100%', maxWidth:380, background:'linear-gradient(135deg, rgba(21,29,39,0.98), rgba(15,20,28,0.98))', border:'1px solid rgba(255,255,255,0.08)', borderRadius:18, padding:20, boxShadow:'0 20px 60px rgba(0,0,0,0.5)'}}>
              ...
              <form onSubmit={submit} style={{display:'flex',flexDirection:'column',gap:10}}>
                <label style={{fontSize:11, color:'#8ea0b8', fontWeight:700}}>Username
                ...
                {mode==='register' && <label style={{fontSize:11, color:'#8ea0b8', fontWeight:700}}>Email
                ...
                <label style={{fontSize:11, color:'#8ea0b8', fontWeight:700}}>Password
                ...
                {err && <div role="alert" style={{color:'#ff3b4a', fontSize:12, background:'rgba(255,59,74,0.08)', padding:'6px 10px', borderRadius:8, border:'1px solid rgba(255,59,74,0.2)'}}>{err}</div>}
                ...
                <span style={{fontSize:10, color:'#5b728c', textAlign:'center'}}>JWT mock • persisted to localStorage • backend /api/auth/login if available</span>
              </form>
            </motion.div>
          </motion.div>
        )}
...
export function RequireAuth({children, fallback}){
  ...
  if(!isAuthenticated) return fallback || <div style={{padding:20,color:'#5b728c'}}>Login required</div>
  return children
}
