...
export default function MarketOverview({ data, history = [] }){
  ...
  const cards = [
    { label:'Advancing', value: data.advancing, sub:`Declining ${data.declining} • Unchanged ${data.unchanged}`, color:'#10b981', icon:'↗', bg:'linear-gradient(135deg, rgba(16,185,129,0.12), rgba(16,185,129,0.04))', trendKey:'advancing' },
    { label:'Above VWAP', value: data.aboveVWAP, sub:`Below ${data.belowVWAP} • VWAP skew`, color:'#2563eb', icon:'◈', bg:'linear-gradient(135deg, rgba(37,99,235,0.12), rgba(37,99,235,0.04))', trendKey:'aboveVWAP' },
    { label:'Breakouts', value: data.breakouts, sub:`Breakdowns ${data.breakdowns} • Momentum`, color:'#f59e0b', icon:'⚡', bg:'linear-gradient(135deg, rgba(245,158,11,0.12), rgba(245,158,11,0.04))', trendKey:'breakouts' },
  ...
  return (
    <div style={{display:'flex', flexDirection:'column', gap:10}}>
      <div className="overview">
        {cards.map(c=>{
          // Real rolling history from the same 15s overview poll App.jsx
          // already runs -- no extra fetch, just a client-side buffer of
          // past snapshots, giving each stat card a genuine multi-point
          // trend instead of a single number.
          const trend = c.trendKey ? history.map(h=> h[c.trendKey]).filter(v=> v!=null) : []
          return (
          <div key={c.label} className="ov-card" style={c.bg?{background:c.bg}:null}>
            ...
            <div style={{display:'flex', alignItems:'baseline', gap:8}}>
              ...
              {trend.length>=2 && <Sparkline data={trend} width={44} height={16} />}
            </div>
